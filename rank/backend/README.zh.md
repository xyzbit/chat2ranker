...
`rankd` 冻结 DatasetVersion、AgentVersion 与 EvaluatorVersion，再把 Run 展开为 Case × Trial 记录。每个 Trial 先提交候选调用并执行必需的确定性检查，仅在需要时提交独立的 Rubric Judge 调用；默认每个用例运行五次。Rank 分离质量失败、基础设施失败与评分失败，聚合有效 Trial 通过率、稳定用例、pass^3，以及候选/评分成本，并且只保存 Execution 与 Artifact 引用。
