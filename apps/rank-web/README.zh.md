...
该组合挂载 Rank Control Host 插件、持久化实验 Session 与 Rank 实验 Skill。Session 可以通过工具创建或选择不可变数据集与 Agent 版本。React UI 渲染聊天、A2UI 准备与确认卡片、SSE 实时进度、运行摘要、失败用例和受控 Artifact。`rankd` 把被测 Agent 提交给独立 Execution Service，它们不会在 Control DSH 进程内运行。
