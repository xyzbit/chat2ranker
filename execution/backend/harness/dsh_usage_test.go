...
func TestReadDSHAccountingAggregatesNativeUsageAndResolvedModel(t *testing.T) {
	home := t.TempDir()
	...
	usage, provider, model, err := readDSHAccounting(home)
	if err != nil {
		t.Fatal(err)
	}
	...
	if provider != "deepseek-official" || model != "deepseek-v4-flash" {
		t.Fatalf("unexpected resolved runtime: %s/%s", provider, model)
	}
}

func TestReadDSHAccountingKeepsUsageForConnectionProvider(t *testing.T) {
	home := t.TempDir()
	path := filepath.Join(home, "sessions", "workspace", "session-one", "session.jsonl.zstd")
	...
	usage, provider, model, err := readDSHAccounting(home)
	if err != nil || usage.InputTokens != 20 || provider != "deepseek-official" || model != "deepseek-v4-flash" {
		t.Fatalf("unexpected accounting: usage=%#v provider=%s model=%s err=%v", usage, provider, model, err)
	}
}
